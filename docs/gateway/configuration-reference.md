...
- `controlUi.github.token`: optional SecretRef-backed service credential for Control UI GitHub previews and project discovery. When omitted, the shipped `GH_TOKEN` then `GITHUB_TOKEN` Gateway environment fallback remains active. This credential is separate from `tools.github` agent identities; an unavailable preview credential does not consume or fall back to an agent profile.
